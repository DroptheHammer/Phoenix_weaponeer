...
interface AttackPreviewMapProps {
  ...
  /** Drawn draggable while the IP is a custom point. */
  customIp?: Coordinates;
  onMoveCustomIp: (point: Coordinates) => void;
  /** While true, the next click on this map places the custom IP. */
  picking: boolean;
...
export function AttackPreviewMap({
  ...
  return (
    <div className="relative h-full w-full">
      <MapContainer center={ll(center)} zoom={9} className={`h-full w-full ${picking ? 'cursor-crosshair' : ''}`} zoomControl={false}>
        ...
        {customIp && <CustomIpMarker position={customIp} onMove={onMoveCustomIp} interactive={!picking} />}
      </MapContainer>
